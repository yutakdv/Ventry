...
## 2026-07-27 값 변경 공지 — 업종 대표면적 교정 (이슈 #152, 가정 #84)

- **DDL·뷰·계약 변경 없음. 바뀌는 것은 `10_data_core.sql` 의 값뿐이다.**
- `cost.REPRESENTATIVE_AREA_M2` 2건이 파이프라인 업종 분류가 아니라 인허가 **대장 구분**
  (휴게 29.3㎡ · 일반 55.4㎡)으로 산출돼 있었다 → **카페 29.2 → 44.0㎡ · 음식점 55.2 → 51.7㎡**.
- 영향: `initial_cost` 전건 3,300행(`monthly_rent`·`deposit_*`·`premium_*`·`monthly_fixed_cost`·
  `cost_ex/incl_premium_*`)과 `rent.monthly_rent` 1,650행. `location_score` w1~w4 는 불변,
  w5 는 만원 반올림 탓에 미세 변동(ρ=0.999997, 종합점수 순위 이동 최대 5칸·상위 50 집합 동일).
- **부담률·θ 통과 수가 이동한다** — 카페 중앙값 0.251 → **0.380**, θ=0.15 통과 360 → **238**;
  음식점 0.211 → **0.198**, 474 → **519**. θ 는 0.15 단일 유지(스펙 §4-1).
- 조치: 실테이블 기준 부담률·후보 수 기대값을 하드코딩한 테스트가 있으면 갱신. 현재 저장소
  기준으로는 해당 없음(`backend/src` 전수 grep에서 29.2·55.2·0.250·0.251 히트 0).
- **1억 예산 카페 후보가 줄어든다** — 권리금 포함 **하한** 기준 1,628 → **1,253**,
  **상한** 기준 1,009 → **100**. 음식점은 10 → 10 으로 불변(초기비용 중앙값이 애초에 1.3억이라
  1억 예산 후보가 원래 10곳뿐인 별개 문제). 데모 시나리오에서 「적합」 카페가 얇아지는지
  확인이 필요하다 — 이슈 #152 「리스크」 항목.
