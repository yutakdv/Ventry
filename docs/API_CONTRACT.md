...
**상태: 확정** — 본 문서가 API 필드·타입·의미의 단일 기준이다.
...
- 판정 enum: `FIT`(적합) / `CONDITIONAL`(조건부 적합) / `CAUTION`(유의) / `OUT_OF_SCOPE`(범위 외)
  — 화면 문구는 용어 컴플라이언스 규정(PROJECT_RULES §2) 준수
...
{ "form": { "age": 32, "capital": 5000,
            ...
            "collateral_available": true,    // 담보 제공 가능 — 현재 판정 미사용, 조달 검증 예약 필드
            "monthly_investable": 250,       // 월 투자 가능액(만원) — 상환 여력 상한
...
// LLM 실파싱이 없기 때문이다(가정 #63). `"llm"` 은 실파싱을 붙인 뒤에야 나간다.
```
...
- **프리뷰 주체는 `/budget`이다.** 화면 2 시점에는 세션 B₀가 아직 미확정이라
...
{ "data_as_of": "2026-Q1",
  ...
  "summary": { "avg_rent": 309, "avg_sales": 2100 }, // 만원. **범위 외를 제외한 후보**의 평균
                                                     // (= 화면 목록에 남는 집합). 남는 후보가
                                                     // 0곳이면 **필드 생략** (D15, non_null)
...
// ★2026-07-26 — 두 플래그가 실제 수행 여부를 반영한다. LLM 검증이 돌고 그 응답이 검증기를
...
- **정렬·필터 쿼리 파라미터는 두지 않는다.** `score`·`monthly_rent`·`est_sales`·
  ...
  "<100ms 재계산 체감" 목표에 유리하다. 후보 풀이 커져 페이징이 필요해지는 시점에 재검토한다.
- 점수 등급 구간(90+/80/70/60) 매핑은 프론트 소관.
- **`areas[]`에 geometry(경계 좌표)는 싣지 않는다 — 계약 밖 정적 자산이다 (2026-07-29 결정).**
  상권 경계와 한국부동산원 조사 구획 경계는 세션·업종·예산과 무관한 불변 자산이라, 응답에
  넣으면 슬라이더를 움직일 때마다 같은 폴리곤이 재전송된다(전량 인라인 시 gzip 약 308KB가
  매 요청에 추가). 배치가 구운 `/geo/area-scope.v1.json`(872KB · gzip 202KB)을 프론트가
  세션당 1회 받아 캐시하며, 조인 키는 `area_code`와 `rent_source.district`로 **둘 다 이미
  이 계약 안에 있다**. 서버는 「상위 100」이라는 개념을 갖지 않으므로(그것은 FE 상수다)
  이 결정은 위의 「정렬·필터 쿼리 파라미터를 두지 않는다」와 정합한다. 가정 #95·#96 참조.
  ⚠️ `rent_source.district`는 폴백 행에서 **`null`** 로 나간다(적재본이 SQL NULL,
  `CandidateRowMapper:72`가 `orEmpty()` 없이 통과). 화면은 항상 존재하는 `fallback`
  불리언으로 분기한다.
...
// insight (T1 예) — insight_id는 refine 이벤트의 교체 대상 키
{ "insight_id": "i-1", "type": "T1", "headline": "…",
  ...
  "funding": { "name": "…", "amount_max": …, "rate": 2.5, "rate_type": "fixed", "term_assumed": 60,
               ...
               "source_quote": { "text": "…", "org": "…", "doc": "…", "date": "…" } },
                                             // 청크 없으면 필드 생략
  "disclaimer": true }
// ★2026-07-29 — `funding` 에는 `data_as_of` 가 없다. 공통 규약(문서 상단)의 「금융상품 응답에
// `data_as_of` 필수」와 어긋나 보이지만, 이 블록은 **인사이트의 근거 상품**이지 사용자가 고르는
// 상품 카드가 아니어서 화면이 상품별 기준일을 쓰지 않는다. 이 화면의 기준일은 `POST /budget`
// 응답(D9)의 값을 화면이 상시 표기한다 — 필드를 늘리지 않고 규약의 예외를 명시하는 쪽을 택했다.
// refine
{ "insight_id": "…", "headline": "…" }
// done
...
| 코드 | 라벨 | 내용 | 예선 구현 |
|---|---|---|---|
| `A1` | 예산 | 상·하향 통합 해석적 프론티어. 계획과 무관하게 항상 실행 | ✅ |
| `A2` | 지역 확장 | 인접 자치구별 프론티어 재계산 | ❌ 미구현 |
| `A3` | 업종 스왑 | 타 업종 프론티어 재계산 | ❌ 미구현 |
| `A4` | 권리금 조건 | `cost_ex_premium` 기준 프론티어 (A1의 부산물) | ✅ |

- **A2·A3 는 예선 스코프에서 미구현이다**(계획된 축소, 가정 #47). `ExploreService.plan` 이 LLM 이 고른
  축에서 이 둘을 제거하므로 **`plan.axes` 에 실리지 않는다**.
...
- **★2026-07-28 — `plan` 의 두 필드는 출처가 다르다**: `axes` 는 LLM 이 대화 맥락(`concerns`)으로
  ...
  소개하지 않는다. LLM 산출 전환은 검토 후 기각했다 — 가정 #90.
- **`marginal_payment`(월 상환액 증분, 만원)은 고정금리 근거일 때만 실린다.** 근거 상품이
  변동금리(`rate` 생략·`rate_type`=`variable`)면 서버는 월 상환액 m을 지어내지 않고(§0-1)
...
{ "verdict": "CONDITIONAL", "gap_amount": 1320,
  "matching_products": [ { "name": "…", "amount_max": 3000,
      ...
      "source_quote": { "text": "만 39세 이하 예비창업자로서…", "org": "소진공",
                        "doc": "○○공고", "date": "2026-06" } } ],
  "risk_review": { "objection_text": "…", "applied": true, "skipped": false } }
```

- **`risk_review.skipped` 는 세 경로 모두에서 항상 실린다.** `FinanceDtos.RiskReview` 의 두 boolean 은
  원시형이라 non_null 직렬화에서도 생략되지 않는다.

- **`source_quote`가 실제로 채워진다.** `finance_product.doc_chunk_ref`
  → `finance_doc_chunk` **id 직접 조회**(LEFT JOIN 1회)이며 유사도 검색·벡터DB는 쓰지 않는다.
  세 경로(`check-area`·`scenarios`·`explore`) 모두 같은 조회를 쓴다.
  - **적재 26건 중 22건이 청크를 보유한다(0.846 · 청크 16개).** 나머지 4건은
    **인용 불가 문서**(KB 4상품 — F-000·F-001·F-002·F-003)로 지정했다:
...
- `GET /api/health` → `{ "status": "ok" }` (계약 고정 응답, 이미 구현)
  - ⚠️ **상수 응답이다 — DB 상태를 반영하지 않는다.** DB 가 완전히 내려간 상태에서도 200 `ok`
    이므로(QA 실측), **오케스트레이션·CI 스모크는 이 값을 게이트로 쓰지 않는다.** 그쪽은
    DB 인디케이터가 붙은 `GET /actuator/health` 를 본다(DB 미가용 시 503 DOWN 실측 확인).
- 오류: HTTP 4xx/5xx + `{ "error": { "code", "message" } }`

  | code | HTTP | 발생 |
  |---|---|---|
  | `INVALID_REQUEST` | 400 | 본문·쿼리 파라미터 규격 위반 (타입·범위·필수 누락) |
  | `SESSION_NOT_FOUND` | 404 | 세션 없음(만료 포함) |
  | `AREA_NOT_FOUND` | 404 | 없는 상권 코드 |
  | `NOT_FOUND` | 404 | 없는 경로 |
  | `METHOD_NOT_ALLOWED` | 405 | 허용되지 않은 HTTP 메서드 (`Allow` 헤더 동반) |
  | `PAYLOAD_TOO_LARGE` | 413 | 요청 본문 256KB 초과 |
  | `UNSUPPORTED_MEDIA_TYPE` | 415 | `Content-Type` 이 `application/json` 이 아님 |
  | `TOO_MANY_SESSIONS` | 429 | 동시 보관 세션 상한(10,000) 도달 |
  | `INTERNAL_ERROR` | 500 | 그 외. 응답 본문에 내부 타입·스택을 싣지 않는다 |
...
|---|---|
| D0 | 초안 작성 (스펙 §6 전사) |
| D0 (7/20) | 초안 공백 5건 식별 (용어·단위·scenarios SSE·risk_review·parse_source) |
| D0 (7/20) | 5건 전부 확정 반영: 판정 enum `CAUTION`(유의), 만원 단위(스펙 §6 정정), scenarios SSE 스키마, recommend `risk_review`, `parse_source` — 근거 DECISIONS.md |
| D2 (7/21) | **FE 검토 의견 6건 반영** — ①diagnose 폼 3필드 ②budget 프리뷰 응답 ③recommend `score`·`total_count`·`summary`·원자재 3종 ④scenarios 예산 범위·상품 `amount_max`/`rate`/`data_as_of` ⑤explore `axis_labels`·`current_budget` ⑥결과 저장 API = 스코프 외 회신. 근거 DECISIONS.md §8~§11 |
| D6 (7/24) | **금융상품 `rate` nullable + `rate_type`·`rate_note` 추가** — 정책자금 변동금리("기준금리+가산")를 고정 숫자로 조작하지 않고 원문 그대로 기록. `finance_product` DDL·`20_finance.sql` 반영, BE는 `rate` NULL 허용 파싱 필요. 근거 assumptions #28 |
| D8 (7/25) | **위 D6 변경 확정** + **FE 조건 4건 반영**: ①JSON 예시 3곳 `rate_type`·`rate_note` ②분기 키 `rate_type`(항상 존재) 명문화 ③`rate_note`·폴백 문구 서버 단일 통제 ④변동금리 `marginal_payment` 생략+`marginal_payment_note`·`matching_products` `amount_max` desc 고정(금리 정렬 금지). BE-05는 변동금리 m 미산출. 데이터 검수 게이트 2건(F-002·F-010 `fixed`+`rate` NULL, F-010 `rate_note` 비금리)은 assumptions #30 등재 |
| D9 (7/27) | **코드리뷰 조치 반영**: ①`POST /budget` 응답에 **`data_as_of` 추가** — 화면 3이 기준일을 표기할 원천이 없던 유일한 화면(④) ②`total_count` 주석 정정 — 값은 그대로 두고 「후보 풀 전체 수(범위 외 포함)」로 의미를 사실에 맞춘다(값을 바꾸면 랜딩 지표 1,061까지 움직인다) ③`burden_ratio` 는 추정매출 결측 시 **필드 생략**(문자열 `"Infinity"` 가 나가던 것, #104 ⑤) ④`marginal_payment_note` **서버 송출 이행** — D8 에서 합의됐으나 미구현이던 필드 |
| D13 (7/29) | **§6 인용 커버리지 서술 정정** — 「적재 26건 전건이 청크를 보유」가 작성 다음 날(7/27) 사실이 아니게 됐는데 문서만 남아 있었다. 실측 **22/26(0.846) · 청크 16개**로 고치고, KB 4상품(F-000~F-003)이 비어 있는 이유(원문에 자격 문단 부재 → 날조 금지)와 **`source_quote` 부재가 정상 응답**이라는 점을 명문화. 근거 assumptions #73·#85 |
| D12 (7/28) | **미기재 규격 2건 명문화 (이슈 #160~#163 · #171~#173)**: ①**§5 `plan.rationale` 예시 정정** — 예시가 「대화 맥락 기반: 권리금 축 우선 검토」라 LLM 산출을 시사했으나 실제로는 **서버 템플릿 상수 2개 중 하나**다. 실제 출력으로 교체하고, `axes`(LLM 산출)와 `rationale`(서버 템플릿)의 출처가 다르다는 사실을 본문에 명시. LLM 전환은 기각 — 가정 #90 ②**§2 SSE `composition`(구간) → §3 `POST /budget` `composition`(단일값) 변환 규칙 기재** — 「자기자본 우선 → 상품 한도 순 그리디」가 FE `lib/composition.ts` 에만 있었다. 이 값은 표기용이 아니라 `UsedLimits.byProduct` 를 거쳐 **잔여 한도 계산에 실제로 쓰이므로**(같은 예산에서 T1 근거 상품이 갈린다) 클라이언트를 다시 구현하면 결과가 조용히 달라진다 |
| D14 (7/29) | **리뷰 조치 문서 정정 3건** — ①**§5 축 표에 A2·A3 미구현 표시** 추가(런타임은 `plan.axes` 에서 이미 제외돼 있었고 표에만 표시가 없었다, BE 리뷰 N-06) ②**§6 예시에 `risk_review.skipped` 반영** — 원시형 boolean 이라 non_null 직렬화에서도 생략되지 않으므로 세 경로 모두 항상 싣고 있었다(BE 리뷰 N-05·계약 리뷰 P2-2) ③**§시스템 계약에 `error.code` 목록 표 신설** + `/api/health` 가 DB 상태를 반영하지 않는다는 사실과 오케스트레이션·CI 게이트가 `/actuator/health` 를 본다는 점 명문화(BE 리뷰 C-01·M-06 · QA 리뷰 Q-03-b) |
| D15 (7/30) | **`summary` 모집단 정정 + 0곳 생략 (실사용 점검 2026-07-29)** — 종전 서버는 `summary` 를 **후보 풀 전체**로 평균해서, 예산을 내려 추천이 0곳이 된 화면에서도 「평균 환산 임대료 221만원 · 평균 추정 매출 1,042만원」이 그대로 떴다. 실측상 예산을 5,000~15,000만원 어디로 옮겨도 두 값이 미동도 하지 않았다 — 예산이 판정만 바꾸고 집계에는 닿지 않았기 때문이다. 「내 한도로 어디까지」를 말하는 화면에서 한도와 무관한 평균을 한도의 결과처럼 보여 주는 것이라 **모집단을 화면 목록과 같은 집합(범위 외 제외)**으로 맞추고, 남는 후보가 **0곳이면 필드를 생략**한다(0곳의 평균은 0이 아니라 정의되지 않는 값이며, 0을 실어 보내면 화면이 「평균 0만원」을 사실처럼 적는다). FE 는 생략 시 「—」로 표기 |
