...
- **★2026-07-29 — A2·A3 는 예선 스코프에서 미구현이다** (TASKS 리스크 #12 의 예정된 축소,
  가정 #47). `ExploreService.plan` 이 LLM 이 고른 축에서 이 둘을 제거하므로 **`plan.axes` 에
  실리지 않는다** — 런타임 불일치가 아니라 계약 표에만 표시가 없던 것이다 (BE 리뷰 N-06).
...
- **★2026-07-29 — `risk_review.skipped` 는 세 경로 모두에서 항상 실린다.** 예시에 `applied` 만
  적혀 있었으나 `FinanceDtos.RiskReview` 의 두 boolean 은 원시형이라 non_null 직렬화에서도
  생략되지 않는다. §4 에는 이미 정의돼 있었고 런타임 동작도 그대로다 — 예시만 사실과 달랐다
  (BE 리뷰 N-05 · 계약 리뷰 P2-2).
...
- `GET /api/health` → `{ "status": "ok" }` (계약 고정 응답, 이미 구현)
  - ⚠️ **상수 응답이다 — DB 상태를 반영하지 않는다.** DB 가 완전히 내려간 상태에서도 200 `ok`
    ...
    계약 필드를 바꾸지 않고 판단 근거만 옮긴 것이다 (QA 리뷰 Q-03-b · BE 리뷰 M-06).
- 오류: HTTP 4xx/5xx + `{ "error": { "code", "message" } }`
  ...
  - **★2026-07-29 — 표 신설.** 코드 목록이 계약에 없어 400/404/405/415 5종이 500 으로 나가던
    것이 문서상 위반인지 판단할 근거가 없었다. 응답 봉투는 그대로이므로 **계약 변경이 아니라
    이미 규정된 「4xx/5xx + error 봉투」의 구체화**다 (BE 리뷰 C-01).
