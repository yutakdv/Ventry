...
- [ ] 이중 필터 재료: 부담률 = 환산임대료(부동산원 상권) ÷ 추정매출(행정동단위구역) ≤ θ
      ⚠️ **부담률은 컬럼으로 굽지 말고 분모·분자를 그대로 적재**한다 — BE가 파생 계산하며
      두 원자재는 화면에도 그대로 표기된다(계약 `monthly_rent`·`est_sales`, DECISIONS.md §10).
      **일평균 유동인구(`daily_floating`)도 정규화 전 원값으로 함께 적재** (계약 표기용)
...
- [ ] 원문 문단 청크 → 청크 테이블 적재, `doc_chunk_ref` 연결 (BE-06 원문 인용 재료)
...
- [ ] SHAP summary plot + 축별 기여 방향 vs 설계 부호 일치 수 (판정 축 3개 → n/3, assumptions #35 ⑤)
- [ ] **게이트 사전 등재 확인**: A(R²≥0.30 ∧ ρ≥0.60 ∧ 방향일치비율≥0.8) / B(R²≥0.15 ∧ (ρ≥0.45 ∨ 방향일치비율≥0.8)) / C(미수록)
- [ ] 한계 명기 문구(부록 2 하단 고정) 포함
- **DoD**: 재현 스크립트 + 프로토콜 등재 (실행·판정은 AI-09)

## AI-09 (D11~12) — 최종 평가·게이트 판정 ★CP5

- [x] `make eval` 최종 실행 → 게이트 A/B/C 판정 → **게이트 B**(순위상관 경로, R² 0.178 · ρ 0.514)
- [x] B 판정에 따라 부록 2 축약 + 경로 병기(`gate_path`) — SHAP 방향 0/3축 미달을 감추지 않는다
- [x] 부록 1·2 수치 확정 → CM-05 기술설명서에 전달. 자격 매칭 행은 BE #77 산출
      (P/R/F1 = 1.000, 프로필 14종 × 상품 26건 = 364쌍)
