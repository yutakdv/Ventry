...
public class CacheConfig {
     ...
     *
     * <p>{@code reviews}·{@code refines} 는 성격이 다르다 — 리스크 검증 반박문(BE-05·#96)과
     * 인사이트 언어화(BE-06 ③)이며 <b>LLM 왕복을 줄이려는</b> 캐시다. 키가 각각 입력 사실
     * 문자열과 템플릿 본문이라 내용이 같으면 같은 결과가 나오고, 슬라이더가 예산을 되돌릴 때
     * 왕복이 사라진다. 최대 크기를 두는 이유도 여기 있다: 예산·후보 조합만큼 키가 늘 수 있어
     * 무한정 쌓게 두지 않는다.
     */
    @Bean
    public CacheManager cacheManager() {
        CaffeineCacheManager manager =
                new CaffeineCacheManager("candidates", "products", "reviews", "refines");
        manager.setCaffeine(
