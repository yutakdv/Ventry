 ...
 * (실제로 방이동먹자골목 198만원 ↔ 매물 호가 250만원 오해가 팀 내에서 발생했다 — 이슈 #151).
 *
 * ⚠️ **배치 상수의 복제본이다.** 원천은 `ai/batch/preprocess/cost.py:11` 의
 ...
 *
 * ⚠️ **이 값은 곧 바뀐다.** 이슈 #152 에서 두 상수 모두 인허가 '대장 구분' 으로 산출된 것이
 * 확인됐다(파이프라인 자신의 KSIC 분류가 아니다). 교정 예정값은 cafe 44.0 · food 51.7 이다.
 * AI 측 반영이 머지되면 아래 두 숫자만 갈아 끼우면 된다.
 */
...
/**
 * 금액 라벨에 붙일 짧은 면적 조건 — "29.2㎡".
 *
 * 임대료 라벨은 **전 화면이 `환산 임대료 (월, 29.2㎡)` 한 형태로 통일**돼 있다. 업종명·평수·
 ...
 *
 * 역산 오차는 `monthly_rent` 가 만원 단위 정수로 반올림된 데서 온다 — 적재본 1,059건 실측으로
 * **최대 0.68% · 평균 0.17%**. 평당 만원 단위(소수 첫째 자리)로 반올림하면 표기 자리 아래로
...
/**
 * 출처 줄에 덧붙일 면적 근거 — "카페 대표면적 29.2㎡(8.8평) 기준".
 *
