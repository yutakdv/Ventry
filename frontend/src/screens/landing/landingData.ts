...
export const PIPELINE = [
  ...
  {
    // 「결과를 문장으로 옮긴다」(refine)는 아직 서버가 하지 않는다 — 실제로 도는 것은
    // 탐색 축 선정(plan)과 리스크 검증 반박 둘이다 (QA_REPORT_INTEGRATION_CM F-7).
    title: 'AI 계획 · 검증',
    desc: 'AI는 어떤 축을 탐색할지 계획하고, 그 결과에 반대 논리를 세웁니다. 수치를 만들거나 다시 계산하지 않습니다.',
  },
]
