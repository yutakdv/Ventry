...
2. [화면 2] 조달 시나리오 2장 → 각 카드의 출처 배지와 공고 원문 인용 주목
   · 보수 6,713만 (범위 5,000~15,000) / 적극 8,398만 (범위 5,000~10,000)
...
3. [화면 3] 예산 8,000만 확정 → 지도·목록에 판정 4단계 마커 + 근거 패널
   ...
   · 상위 후보: 방이동먹자골목 🟢 적합(93) · 신림역 8번 ⚪ 조건부 적합(84)
4. 탐색 카드 자동 전개 → 헤더의 계획 근거는 LLM의 대화 맥락 판단입니다
   (권리금이 걱정이라고 적었으므로 예산·권리금 두 축이 선택됩니다) .. AI [2] 결정공간 탐색
                                                              기술설명서 §6
   ★ 핵심: T1 인사이트의 진입 수와 지속 수 병기.
     "2,770만 원을 추가 확보하면 진입 가능 후보는 342곳에서 1,020곳으로 늘어납니다.
      다만 월 상환 부담 50만 원을 반영하면 지속 안정 후보는 334곳입니다."
     → 차입으로 진입 후보가 3배가 되는데 버틸 수 있는 곳은 출발점(342곳)보다 적습니다.
...
6. (선택) 슬라이더를 6,000만으로 → 재계산 /
   임의 상권 클릭 → 역방향 판정 + 자격 부합 상품 25건(+공고 원문 인용) .. 기술설명서 §5
```
