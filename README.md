...
> **[0-A] 카카오맵 앱키가 없어도 전 동선을 완주할 수 있습니다.**
> `.env` 를 비운 채 그대로 실행하셔도 **진단 → 시나리오 → 판정 → 탐색 → 리스크 검증**이
> 끝까지 돌아갑니다. 앱키가 없을 때 화면 3은 이렇게 됩니다:
...
0. docker compose up --build  →  http://localhost:3000
   (새 클론 기준 최초 --build 약 1분 / 이미지 준비 후 재기동 약 11초)
1. [화면 1] "마이데이터 불러오기" 클릭 (예비창업자 / 만 32세 / 자기자본 5,000만 / 마포 카페)
   → 폼 + 자유 텍스트 진단 결과 확인
                    .. 폼 + 키워드 파싱(**결정적** — 이 화면에는 LLM이 없습니다)
   · `parse_source: form_only` — LLM 3역할(① 탐색 축 선정 ② 언어화 ③ 리스크 반박)은
     4·5번 화면에서 처음 등장합니다
...
2. [화면 2] 조달 시나리오 2장 → 조달 구성 내역의 상품을 펼쳐 출처·기준일·금리 표기 주목
   ...
   · 두 라벨은 금액의 대소가 아니라 조달 수단입니다 —
     보수는 보증형만, 적극은 전체 상품을 후보로 봅니다 ....... 기술설명서 09면
3. [화면 3] 예산 1억 확정 → 지도·목록에 판정 4단계 마커 + 근거 패널
   ...
   · 후보 풀은 **서울 전역**이고, 화면은 진단에서 고른 **자치구(마포구 20곳)를 먼저** 보여줍니다.
     「서울 전체로」 한 번이면 382곳 전부로 돌아갑니다 — 드롭다운에 25개 구의 후보 수가 함께
     있어, 좁혀진 상태에서도 다른 구가 몇 곳인지 사라지지 않습니다
   · 진단의 지역 입력은 **지역 한정 상품의 자격 판정**에도 쓰입니다 (서울 11건 / 서울 밖 1건)
...
6. (선택) 슬라이더를 9,000만으로 → 진입 가능 382 → 114곳으로 재계산 /
   임의 상권 클릭 → 역방향 판정 + 자격 부합 상품 11건(+공고 원문 인용) .. 기술설명서 09면
```
...
> PostgreSQL 베이스 이미지 내려받기 + Gradle·npm 의존성 설치가 선행됩니다. 실측(2026-08-01,
> Apple M5 Pro · macOS · Docker 29.5.3 — 이미지·빌드 캐시를 전부 지운 상태에서 2회 반복)은
> **빌드 47초 + 최초 기동 19초 = 약 66초**, 이미지가 준비된 뒤의 재기동은 **약 11초**였습니다
> (전 컨테이너 healthy 까지). 최초 기동의 19초에는 `db/init/*.sql` 자동 적재가 포함됩니다.
> 회선·머신 사양에 따라 달라지므로 **근사치**로 읽어 주십시오 — 베이스 이미지 내려받기가
> 시간의 대부분이라, 느린 회선에서는 몇 분이 될 수 있습니다. 기다리는 동안 화면이 뜨지 않는
> 것은 정상입니다.
...
> - `VITE_KAKAO_APP_KEY` 를 비웠을 때 화면 3이 어떻게 되는지는 위 **[0-A]** 에 적었습니다.
>   지도만 빠지고 동선·판정·근거는 그대로입니다.

**AI 품질 평가 재현**: 아래 3줄로 기술설명서 부록 A1·A2의 AI 산출물 지표
...
괄호 안은 확인일이다. 추정하지 않았고, 확인하지 못한 항목은 그렇게 적었다.
...
> 좌표는 전부 WGS84로 통일했다(원본 EPSG:5174/5179/5181 → pyproj 변환).
> t-data 지하철역_GEOM(CC-BY)은 폴백 후보였으나 **역사마스터가 WGS84 좌표를 직접 제공해 사용하지 않았고**,
> 쓰지 않은 소스이므로 위 표에서 뺐다(가정 #14).
