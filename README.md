...
2. [화면 2] 조달 시나리오 2장 → 각 카드의 출처 배지와 공고 원문 인용 주목
   · 보수 7,901만 (범위 5,000~15,000) / 적극 1억 440만 (범위 5,000~12,000)
   · 두 라벨은 금액의 대소가 아니라 조달 수단입니다 —
     보수는 보증형만, 적극은 전체 상품을 후보로 봅니다 ....... 기술설명서 §5
3. [화면 3] 예산 1억 확정 → 지도·목록에 판정 4단계 마커 + 근거 패널
   · 진입 가능 후보 382곳 · 조건부 적합 636곳 (임대료 출처·교통 셀이 상시 표기됩니다)
     두 수가 다른 이유: 조건부 적합은 **무권리 매물을 잡아야 열리는 구간**이라 진입 후보에
     들어가지 않지만, 마커 3종에는 포함됩니다
...
> 위 수치는 **2026-07-28 실적재본 실측**입니다 (`develop` `0965fe6` · `docker compose` 구동본).
